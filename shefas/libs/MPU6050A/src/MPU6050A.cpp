...
byte MPU6050A::begin(int gyro_config_num, int acc_config_num){
...
byte MPU6050A::writeData(byte reg, byte data){
...
byte MPU6050A::readData(byte reg) {
  wire->beginTransmission(address);
  wire->write(reg);
...
byte MPU6050A::setGyroConfig(int config_num){
  byte status;
  switch(config_num){
...
byte MPU6050A::setAccConfig(int config_num){
  byte status;
  switch(config_num){
    case 0: // range = +- 2 g
...
void MPU6050A::setGyroOffsets(float x, float y, float z){
  gyroXoffset = x;
  gyroYoffset = y;
  gyroZoffset = z;
}

void MPU6050A::setAccOffsets(float x, float y, float z){
  accXoffset = x;
  accYoffset = y;
  accZoffset = z;
}

void MPU6050A::setFilterGyroCoef(float gyro_coeff){
...
void MPU6050A::setFilterAccCoef(float acc_coeff){
  setFilterGyroCoef(1.0-acc_coeff);
}

/* CALC OFFSET */

void MPU6050A::calcOffsets(bool is_calc_gyro, bool is_calc_acc){
